...
@section('content')
    <div class="page-head">
        <div>
            <h2 style="margin:0;">Approvisionnements et dettes fournisseurs</h2>
            <div class="muted">Une facture fournisseur approuvee augmente le stock et alimente le suivi des montants restant a regler.</div>
        </div>
    ...
    <div class="grid stats-grid" style="margin-bottom:20px;">
        ...
        <div class="card"><div class="muted">Reste a regler</div><div class="stat-value">{{ number_format($summary['open_balance'], 0, ',', ' ') }}</div></div>
            ...
            @forelse ($bills as $bill)
                ...
                <tr>
                    <td>
                        ...
                        @if ($bill->notes)
                            <div class="muted" style="font-size:13px;">{{ $bill->notes }}</div>
                        @endif
                    </td>
                    <td>{{ $bill->bill_date?->format('d/m/Y') }}</td>
                    <td>{{ $bill->due_date?->format('d/m/Y') ?? 'Non renseignee' }}</td>
