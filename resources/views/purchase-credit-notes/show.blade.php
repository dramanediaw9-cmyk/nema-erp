...
@section('title', 'Avoir fournisseur '.$creditNote->credit_note_number)
@section('page-title', 'Avoir '.$creditNote->credit_note_number)

@section('content')
    <div class="page-head">
        <div>
            <h2 style="margin:0;">{{ $creditNote->supplier?->name }}</h2>
            <div class="muted">Avoir du {{ $creditNote->credit_note_date?->format('d/m/Y') }} · Facture {{ $creditNote->bill?->bill_number }}</div>
        </div>
        <div style="display:flex; gap:10px; flex-wrap:wrap;">
            <a href="{{ route('purchase-credit-notes.print', $creditNote) }}" target="_blank" class="button button-secondary">Imprimer l avoir</a>
            <a href="{{ route('purchases.show', $creditNote->bill) }}" class="button button-secondary">Voir la facture</a>
        </div>
    </div>
    ...
    <div class="grid stats-grid" style="margin-bottom:20px;">
        ...
        <div class="card"><div class="muted">Retour stock</div><div class="stat-value" style="font-size:24px;">{{ $creditNote->destock_items ? 'Oui' : 'Non' }}</div></div>
        <div class="card"><div class="muted">Facture source</div><div class="stat-value" style="font-size:24px;">{{ $creditNote->bill?->bill_number }}</div></div>
        <div class="card"><div class="muted">Solde facture apres avoir</div><div class="stat-value">{{ number_format((float) $creditNote->bill?->balance_due, 0, ',', ' ') }}</div></div>
    </div>
    ...
    <div class="split" style="margin-bottom:20px;">
        <section class="card">
            ...
            <div class="table-wrap">
                <table>
                    <thead><tr><th>Produit</th><th>Description</th><th>Quantite</th><th>Cout</th><th>TVA</th><th>Total</th></tr></thead>
                    <tbody>
                    @foreach ($creditNote->items as $item)
                        <tr>
                            ...
                            <td>{{ number_format((float) $item->tax_amount, 0, ',', ' ') }} XOF</td>
        ...
        <section class="card">
            ...
            <div class="grid">
                ...
                <div><strong>Stock sorti</strong><div class="muted">{{ $stockMovements->count() }} mouvement(s)</div></div>
    ...
    <div class="split">
        <section class="card">
            ...
            @empty
                <p class="muted">Aucune sortie stock enregistree sur cet avoir.</p>
            @endforelse
        </section>
