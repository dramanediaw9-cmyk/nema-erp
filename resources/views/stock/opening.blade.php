...
@section('title', 'Stock initial - Nema ERP')
@section('page-title', 'Saisir le stock initial')

@section('content')
    <div class="page-head">
        <div>
            <h2 style="margin:0;">Stock initial unitaire</h2>
            <div class="muted">Pour un chargement de masse, utilise plutot l import CSV depuis le centre d imports.</div>
        </div>
        @allowed('imports.manage')
            <a href="{{ route('imports.index') }}" class="button button-secondary">Importer le stock en lot</a>
        @endallowed
    </div>
    ...
    <form method="POST" action="{{ route('stock.opening.store') }}">
        ...
        <div class="card">
            ...
            <div class="actions">
                ...
                <button type="submit" class="button button-primary">Enregistrer le stock initial</button>
            </div>
        </div>
    </form>
@endsection
